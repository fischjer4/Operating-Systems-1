...
void decrypt(char message[], char key[]){ //OTP decryption method where there are 26 UPPERCASE chars + 1 space = 27 options
    int i = 0; 
    while(message[i] != '\0' && key[i] != '\0'){
        if( ((message[i] - 64) - (key[i] - 64) ) < 0)
            message[i] = 27 - (abs( (message[i] - 64) - (key[i] - 64) ) % 27);
        else
            message[i] = ((message[i] - 64) - (key[i] - 64)) % 27;
...
void setupClient(char* port, int* listenSocketFD, struct sockaddr_in* serverAddress){
	...
	*listenSocketFD = socket(AF_INET, SOCK_STREAM, 0); // Create the socket
	if (*listenSocketFD < 0){ error("DEC SERVER: ERROR opening socket"); exit(1);}

	if (bind(*listenSocketFD, (struct sockaddr *)&(*serverAddress), sizeof(*serverAddress)) < 0){ // Connect socket to port
		error("DEC SERVER: ERROR on binding"); exit(1);}
	if(listen(*listenSocketFD, 5) < 0){ error("DEC SERVER: ERROR on listen"); exit(1);}// Flip the socket on - it can now receive up to 5 connections
...
int sendAll(int socket, char buffer[], size_t length){
    ...
    while (length > 0){ //while we havnt sent all characters keep sending
        ...
        if (charsProcessed < 1) { if(buffer) free(buffer); error("DEC SERVER: ERROR writing to socket"); return -1;}
        ptr += charsProcessed;
...
int recvAll(int socket, char buffer[], size_t length){
    ...
    while (length > 0){ //while we havnt recieved all characters keep pulling
        ...
        if (charsProcessed < 1){ if(buffer) free(buffer); error("DEC SERVER: ERROR recieving to socket"); return -1;}
        ptr += charsProcessed;
        length -= charsProcessed;
    }
    return 1;
...
void talkToServer(const int establishedConnectionFD){
    ...
    if(expectedRead < 0){ error("DEC SERVER: ERROR reading from socket"); return;}
    ...
    // printf("DEC SERVER: expectedRead = %d\n",expectedRead);
	
    if(recvAll(establishedConnectionFD, message, expectedRead) < 0) return; // Get the message from the client
    if(recvAll(establishedConnectionFD, key, expectedRead) < 0) return;
	
    // printf("DEC SERVER: Message: %s      KEY: %s\n",message,key);
    decrypt(message,key); //note: opt_enc.c should have already verified message and key prereqs.
	// printf("DEC SERVER: Message: %s      KEY: %s\n",message,key);
    
    if(sendAll(establishedConnectionFD, message, expectedRead) < 0) return; // Send plaintext message back to the client
...
int main(int argc, char* argv[]){
    ...
    while(1){
        ...
        if (establishedConnectionFD < 0) error("DEC SERVER: ERROR on accept");
        memset(buffer, '\0', MAXSIZE);
        recvAll(establishedConnectionFD, buffer, 10); //verify connection to otp_dec
        if(strncmp(buffer,"JFisch_DEC",10) != 0){
            close(establishedConnectionFD); //Connection is not otp_dec
            error("DEC SERVER: ERROR source other than otp_enc attempted to connect");
        }
        else{
            spawnPid = fork();
            switch(spawnPid){
                case -1:
                    error("DEC SERVER: ERROR Fork() Failed inside of opt_enc_d \n");
                    break;
                case 0: //child
